...
const QuestionListItem = React.memo(({ q, onEdit, onDelete }: { q: Question, onEdit: (q: Question) => void, onDelete: (id: string) => void }) => {
    return (
...
const QuestionList: React.FC<QuestionListProps> = ({ questions, onEdit, onDelete, onDeleteSet, onEditSet }) => {
  ...
  // Derive which folders should be open
  // This avoids calling state setters inside useMemo (performance anti-pattern)
  const effectiveOpenFolders = useMemo(() => {
      // If searching, all folders containing matches are open
      if (searchTerm.trim()) {
          return new Set(groupedQuestions.keys());
      }

      // If user has interacted with folders, use their state
      if (openFolders !== null) {
          return openFolders;
      }

      // Initial state: open the first folder by default
      if (groupedQuestions.size > 0) {
          return new Set([groupedQuestions.keys().next().value]);
      }

      return new Set<string>();
  }, [groupedQuestions, openFolders, searchTerm]);

  const toggleFolder = (codePrefix: string) => {
    setOpenFolders(prev => {
        // Initialize from effective state if first interaction
        const currentSet = prev ?? (groupedQuestions.size > 0 ? new Set([groupedQuestions.keys().next().value]) : new Set());
